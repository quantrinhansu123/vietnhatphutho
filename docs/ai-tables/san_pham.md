...
**`do_li`**: chỉ nhận token `…li` tường minh (từ mã AMIS `- Nli` hoặc tên SX); chứa KG hay ZEM thì không phải độ li (bỏ, suy tiếp hoặc để trống). Sửa dữ liệu cũ: `supabase-san-pham-xoa-do-li-zem.sql`.
