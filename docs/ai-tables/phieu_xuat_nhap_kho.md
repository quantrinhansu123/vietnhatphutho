...
Chi tiết NVL được đọc trực tiếp từ JSON `chi_tiet` của các phiếu định mức đã chọn. Màn hình nhập **chia nhóm NVL chính / NVL phụ** (header + tổng TL từng nhóm); nút **Thêm NVL chính** / **Thêm NVL phụ** thêm dòng trống đúng phân loại. Combobox mã liệt kê từng dòng kho (cùng mã vẫn hiện riêng nếu khác tên hoặc tên sản xuất), không gộp còn 1 dòng theo mã.
...
Với NVL phụ, `gia_tri` trên phiếu trộn là SL theo ĐVT gốc và `tong_khoi_luong` là kg đã quy đổi. Phiếu xuất kho dùng `gia_tri` cho **SL CT** và hệ số `tong_khoi_luong / gia_tri` để tính **Quy đổi kg** khi nhập SL thực. Ngoại lệ: ĐVT `kg` → **Quy đổi kg = SL thực** tuyệt đối, không nhân hệ số (FE `resolveLineWeightKg` + `parseWarehouseSlipPayloadItems` + prefill PTĐM force hệ số 1, server `parseWarehouseSlipLines` cũng short-circuit).
Trọng lượng quy đổi được lưu tại `trong_luong_kg` (cả NVL chính: lấy kg/đơn vị từ định mức, lẫn NVL phụ). Lưới xuất NVL có thêm cột **Nhóm VTHH** — chỉ hiện select (Rỗng/Đặc/Sóng) với Băng Dính/Tem, dòng khác hiện `—`; đổi VTHH cập nhật luôn `normWeightPerUnitKg`.
Ô **Tên sản xuất** NVL là select mở lên trên (`openUpward`, `MaterialProductionNamePair`), lọc **theo đúng mã NPL của dòng**; đổi tên SX chốt đúng dòng kho (`materialId` + tên/ĐVT/nhóm phụ/hệ số theo `match.id`). Bảng NVL chính và NVL phụ trên mẫu in/in lại đều có cột **Trọng lượng (kg)** và dòng tổng kg riêng; cuối phiếu in (nhiều máy) thêm **TỔNG TL NVL CHÍNH TOÀN PHIẾU** và **TỔNG TL NVL PHỤ TOÀN PHIẾU**. Màn lập phiếu và chi tiết lịch sử hiển thị tổng TL chính/phụ riêng.
...
- Ghi sổ SP: sau phiếu nhập TP → `nhap_kho` với `loai_kho='thanh_pham'` và hệ số 1 SP (`trong_luong_kg_mot_sp`, `so_m2_mot_sp`, `so_m_dai_mot_sp`). Tổng dòng vẫn ở phiếu (xem [nhap_kho.md](./nhap_kho.md)).
- UI phiếu (`warehouseKind=san_pham`): **nhập** — Ngày phiếu riêng; khối viền vàng **Lọc lệnh sản xuất** (ngày lọc lệnh + chọn nhiều máy) → tick lệnh → nạp **mã SP + tên sản xuất (tên ghép)** + quy đổi **kg / m² / m dài**; ghi `nhap_kho` (`ma_sp`, `ten_sp` = tên ghép, hệ số 1 SP); **không dùng ca**. **Xuất** — trong **Chi tiết sản phẩm** mỗi dòng `SearchableSelect` chọn **1 SP** từ tồn (`/api/nhap-kho`); quy đổi lấy hệ số 1 SP trên sổ; cột Mã · Tên · ĐVT · SL CT · SL thực · kg · m² · m dài · Giá · Thành tiền (**không** khối multi-select “Thêm từ tồn kho hệ thống”; **không dùng ca**).
