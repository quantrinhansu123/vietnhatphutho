...
const sqlPath = path.join(process.cwd(), 'supabase-chi-phi-nhan-cong.sql');
...
async function run() {
  ...
  for (const connectionString of urls) {
    ...
    try {
      ...
      console.log('SUCCESS: Table chi_phi_nhan_cong created or updated successfully.');
      ok = true;
      await client.end();
      break;
    } catch (e) {
  ...
  if (!ok) {
    console.log('NOTICE: Could not connect directly to postgres. You can run supabase-chi-phi-nhan-cong.sql in Supabase SQL Editor.');
  }
}
