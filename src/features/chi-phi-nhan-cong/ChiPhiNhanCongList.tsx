...
interface ChiPhiNhanCongListProps {
  ...
  onOpenDinhGia: (record?: ChiPhiNhanCongRecord, tab?: 'thang' | 'nam') => void;
}
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          {/* Nút Định giá theo năm + Thêm mới */}
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => onOpenDinhGia(undefined, 'nam')}
              className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-indigo-300 bg-indigo-50 px-4 text-xs font-bold text-indigo-800 shadow-sm transition hover:bg-indigo-100 active:scale-95"
              title="Tổng hợp và so sánh định mức nhân công (đồng/kg) năm này với năm trước theo từng máy"
            >
            ...
            <table className="w-full text-left text-xs text-slate-600">
              <thead className="border-b border-slate-200 bg-slate-50/80 text-[11px] font-bold uppercase tracking-wider text-slate-700">
                <tr>
                  ...
                  <th className="px-3.5 py-3 text-center w-36">Thao tác</th>
                </tr>
              </thead>
                    ...
                    <tr key={rec.id} className="transition hover:bg-slate-50/80">
                      ...
                      <td className="px-3.5 py-3 text-center whitespace-nowrap">
                        <div className="inline-flex items-center gap-1">
                          ...
                          <button
                            type="button"
                            onClick={() => onOpenDinhGia(rec)}
                            title={`Định giá tháng ${rec.thang}/${rec.nam} so với tháng trước`}
                            className="rounded p-1 text-slate-500 hover:bg-amber-50 hover:text-amber-700 transition"
                          >
