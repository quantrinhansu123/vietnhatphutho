  ...
  return <div className="fixed inset-0 z-[250] flex items-center justify-center bg-black/45 p-3">
    <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
      ...
      <div className="grid grid-cols-1 gap-4 p-5 md:grid-cols-2">
        ...
        <label><span className="mb-1 block text-xs font-black uppercase text-zinc-500">Đơn vị tính *</span><input value={form.unit} onChange={e => setForm(prev => ({ ...prev, unit: e.target.value }))} className={fieldClass} placeholder="tấm, m, m2..." /></label>
        <div />
...
export function ProductConversionsPanel({ onBack }: { onBack: () => void }) {
  ...
  return <div className="min-h-full bg-white font-sans [&_button]:cursor-pointer [&_tbody_tr]:cursor-pointer [&_tbody_tr]:transition-colors [&_tbody_tr:hover]:bg-emerald-50">
    ...
    <div className="p-4 md:p-6"><input value={key} onChange={e => setKey(e.target.value)} className={`${fieldClass} mb-3 max-w-xl`} placeholder="Tìm mã SP, mã AMIS, tên sản phẩm hoặc đơn vị..." />{importProgress && saving && <div className="mb-3 rounded-lg border border-blue-200 bg-blue-50 p-3"><div className="mb-2 flex justify-between text-sm font-bold text-blue-800"><span>Đang xử lý CSV</span><span>{importProgress.processed}/{importProgress.total} dòng</span></div><div className="h-2 overflow-hidden rounded-full bg-blue-100"><div className="h-full bg-blue-600 transition-all" style={{ width: `${importProgress.total ? Math.round(importProgress.processed / importProgress.total * 100) : 0}%` }} /></div><p className="mt-2 text-xs font-semibold text-blue-700">Thêm {importProgress.created} · Cập nhật {importProgress.updated} · Lỗi/bỏ qua {importProgress.failed}</p></div>}{message && <p className="mb-3 rounded-lg bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700">{message}</p>}{error && <p className="mb-3 rounded-lg bg-red-50 px-3 py-2 text-sm font-bold text-red-700">{error}</p>}
      <div className="max-h-[calc(100vh-260px)] overflow-auto rounded-xl border"><table className="min-w-[1450px] w-full text-left text-xs"><thead className="sticky top-0 z-10 bg-zinc-950 text-white"><tr>{['Mã SP','Mã AMIS','Tên sản phẩm','ĐVT','Rộng tấm','Dài tấm','Rộng cuộn','Dài cuộn','m²','kg/m dài','kg/m²','kg/tấm','Thao tác'].map(h => <th key={h} className="px-3 py-3 font-black">{h}</th>)}</tr></thead><tbody>{loading ? <tr><td colSpan={13} className="py-10 text-center"><Loader2 className="mx-auto animate-spin" /></td></tr> : filteredRows.length === 0 ? <tr><td colSpan={13} className="py-10 text-center font-bold text-zinc-400">Không có dữ liệu phù hợp.</td></tr> : filteredRows.map(row => <tr key={row.id} className="border-t"><td className="px-3 py-2 font-black">{row.maSp || '—'}</td><td className="px-3 py-2">{row.maAmis || '—'}</td><td className="max-w-xs px-3 py-2 font-semibold">{row.tenSp || '—'}</td><td className="px-3 py-2 font-black">{row.donViTinh}</td>{[row.khoTamRongM,row.khoTamDaiM,row.khoCuonRongM,row.khoCuonDaiM,row.dienTichM2,row.trongLuongKgMDai,row.trongLuongKgM2,row.trongLuongKgTam].map((v,i) => <td key={i} className="px-3 py-2 text-right">{fmt(v)}</td>)}<td className="px-3 py-2"><div className="flex gap-1">{canEdit && <button title="Sửa" onClick={() => { setEditing(row); setModalOpen(true); }} className="rounded border p-2 text-blue-600"><Pencil className="h-4 w-4" /></button>}{canDelete && <button title="Xóa" onClick={() => void remove(row)} className="rounded border p-2 text-red-600"><Trash2 className="h-4 w-4" /></button>}</div></td></tr>)}</tbody></table></div>
