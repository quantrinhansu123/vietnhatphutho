    ...
    <div className="dot-san-xuat-print-sheet">
      <div className="dot-san-xuat-print-doc">
        ...
        <table className="dot-san-xuat-print-table">
          <thead>
            <tr>
              <th style={{ width: '16%' }}>Đợt</th>
              <th style={{ width: '24%' }}>Số công (Trực / Đầu máy / Cuối máy)</th>
              <th style={{ width: '12%' }}>Số nhân công (số công)</th>
              <th style={{ width: '16%' }}>Tổng chi phí nhân công</th>
              <th style={{ width: '16%' }}>Đơn giá nhân công/kg đợt này</th>
              <th style={{ width: '16%' }}>BQ vật tư + nhân công/kg</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>{data.ten_dot}</td>
              <td className="dot-san-xuat-print-center">
                {formatKg(num(data.so_cong_truc))} / {formatKg(num(data.so_cong_dau_may))} / {formatKg(num(data.so_cong_cuoi_may))}
              </td>
              ...
              <td className="dot-san-xuat-print-right">{formatUnitPrice(d.donGiaNhanCong)}</td>
              <td className="dot-san-xuat-print-right">{formatUnitPrice(d.bqVatTuNhanCong)}</td>
            </tr>
          </tbody>
        </table>

        <h2 className="dot-san-xuat-print-subtitle">Lệnh sản xuất trong đợt ({data.lenh_sx.length})</h2>
        {data.lenh_sx.length === 0 ? (
          <p className="dot-san-xuat-print-muted">Không có lệnh sản xuất nào được gom vào đợt.</p>
        ) : (
          <table className="dot-san-xuat-print-table">
            <thead>
              <tr>
            ...
            <tbody>
              {data.lenh_sx.map((lenh, index) => (
                <tr key={`${lenh.ma_lenh_sx}-${index}`}>
                  <td className="dot-san-xuat-print-center">{index + 1}</td>
                  <td>{lenh.ma_lenh_sx || '-'}</td>
                  <td className="dot-san-xuat-print-center">{formatPrintDate(String(lenh.ngay_bat_dau || '').slice(0, 10))}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <h2 className="dot-san-xuat-print-subtitle">Phiếu xuất kho NVL trong đợt ({data.phieu_xuat_codes.length})</h2>
        {data.phieu_xuat_codes.length === 0 ? (
          <p className="dot-san-xuat-print-muted">Không có phiếu xuất kho NVL nào khớp máy + khoảng ngày.</p>
        ) : (
          <p className="dot-san-xuat-print-muted">{data.phieu_xuat_codes.join(' · ')}</p>
        )}
