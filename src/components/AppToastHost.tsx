...
export default function AppToastHost() {
  ...
  return createPortal(
    <div className="pointer-events-none fixed right-4 top-14 z-[80] flex w-[min(24rem,calc(100vw-2rem))] flex-col items-end gap-2">
      <AnimatePresence>
        {toasts.map(toast => (
          <motion.div
            key={toast.id}
            ...
            className={`w-full rounded-2xl border p-3 text-[12.5px] font-semibold leading-relaxed shadow-lg backdrop-blur-xl ${
              toast.type === 'success'
